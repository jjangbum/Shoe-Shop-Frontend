...
const Header = memo(({ loggedIn, setLoggedIn }) => {
  ...
  return (
    <Disclosure>
      {({ open }) => (
        <header className='w-full flex flex-col sm:flex-row px-6 lg:px-40 py-4 fixed z-50 border-b rounded-b-lg border-neutral-300 drop-shadow-sm bg-neutral-50'>
          ...
          <Disclosure.Panel
            className='sm:hidden flex flex-col mt-7 -mb-1 text-center'
            as='div'>
            {loggedIn ? (
              <>
                <Link
